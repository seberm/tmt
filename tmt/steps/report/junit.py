...
class ResultWrapper:
    """
    Adds possibility to wrap the ``tmt.Result`` and dynamically add more attributes which get
    available inside the template context.
    """
    ...
    def set_properties(self, keyval: dict[str, str]) -> None:
...
class ResultsContext:
    ...
    def duration(self) -> int:
        ...
        durations: list[int] = [duration_to_seconds(r.duration) or 0 for r in self._results]
    ...
    try:
        ...
        try:
            ...
            try:
                ...
                phase.warn(str(e))
                phase.verbose('data', xml_data, 'red')
                raise tmt.utils.ReportError(
